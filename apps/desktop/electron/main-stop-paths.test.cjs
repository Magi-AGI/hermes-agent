...
test('backend sidecar path is per-generation (owner nonce folded into the filename)', () => {
  // The helper must accept the owner nonce and include it in the path so a fresh
  // spawn never shares a sidecar file with a prior generation of the same id.
  const helper = mainSrc.match(/function makeDesktopBackendSidecarPath\(([^)]*)\)\s*\{([\s\S]*?)\n\}/)
  assert.ok(helper, 'makeDesktopBackendSidecarPath must exist')
  assert.ok(/ownerNonce/.test(helper[1]), 'helper must take an ownerNonce parameter')
  assert.ok(/ownerNonce/.test(helper[2]), 'helper body must fold the ownerNonce into the path')
  // The spawn path must pass the nonce when deriving the sidecar path.
  assert.ok(
    /makeDesktopBackendSidecarPath\(\s*registryId\s*,\s*ownerNonce\s*\)/.test(mainSrc),
    'spawnPoolBackend must derive a per-generation sidecar path with the owner nonce'
  )
})

test('post-ready sidecar association uses the bounded ownership-safe re-read', () => {
  // The one-shot readJsonFileIfExists(sidecarPath) is replaced by the bounded,
  // matching-nonce-only re-read so a read-before-write race can no longer disown a
  // healthy backend, and a foreign/stale sidecar is never adopted.
  assert.ok(
    /readOwnedSidecarWithRetry\(\s*\{[\s\S]*?ownerNonce[\s\S]*?\}\s*\)/.test(mainSrc),
    'spawn path must associate the sidecar via readOwnedSidecarWithRetry with the owner nonce'
  )
  // The mismatch log survives but is now gated on a genuine non-matching file.
  assert.ok(
    mainSrc.includes('owner nonce did not match registry record'),
    'the disown log line is retained for a genuinely foreign leftover'
  )
})
