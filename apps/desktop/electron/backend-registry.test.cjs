...
// ── owner-nonce sidecar association (checkpoint 097) ────────────────────────

test('sidecarMatchesOwner requires kind + exact owner nonce (never adopts foreign)', () => {
  const nonce = 'gen-abc'
  assert.equal(sidecarMatchesOwner({ kind: 'hermes-desktop-backend', ownerNonce: nonce }, nonce), true)
  // Wrong nonce (a prior/foreign generation) is rejected.
  assert.equal(sidecarMatchesOwner({ kind: 'hermes-desktop-backend', ownerNonce: 'other' }, nonce), false)
  // Wrong kind rejected.
  assert.equal(sidecarMatchesOwner({ kind: 'something-else', ownerNonce: nonce }, nonce), false)
  // Missing pieces / null inputs fail closed.
  assert.equal(sidecarMatchesOwner(null, nonce), false)
  assert.equal(sidecarMatchesOwner({ kind: 'hermes-desktop-backend', ownerNonce: nonce }, ''), false)
  assert.equal(sidecarMatchesOwner({ ownerNonce: nonce }, nonce), false)
})

test('readOwnedSidecarWithRetry returns verified once a matching sidecar appears', async () => {
  const nonce = 'gen-1'
  let reads = 0
  const result = await readOwnedSidecarWithRetry({
    ownerNonce: nonce,
    attempts: 5,
    delayMs: 1,
    sleep: async () => {},
    // Child publishes late: absent for the first two reads, then our sidecar.
    read: () => {
      reads += 1
      if (reads < 3) return null
      return { kind: 'hermes-desktop-backend', ownerNonce: nonce, pid: 4321 }
    }
  })
  assert.equal(result.verified, true)
  assert.equal(result.sidecar.pid, 4321)
  assert.equal(reads, 3, 'stopped polling as soon as the matching sidecar appeared')
})

test('readOwnedSidecarWithRetry never adopts a foreign sidecar and reports it for logging', async () => {
  const stale = { kind: 'hermes-desktop-backend', ownerNonce: 'prior-gen', pid: 9 }
  let reads = 0
  const result = await readOwnedSidecarWithRetry({
    ownerNonce: 'gen-2',
    attempts: 3,
    ...
    read: () => {
      reads += 1
      return stale // a leftover prior-generation file that never matches
    }
  })
  assert.equal(result.verified, false, 'foreign nonce is never adopted')
  assert.equal(result.sidecar, stale, 'last non-matching sidecar surfaced for the mismatch log')
  assert.equal(reads, 3, 'exhausted the bounded attempt budget')
})

test('readOwnedSidecarWithRetry returns unverified+null when the sidecar never appears', async () => {
  const result = await readOwnedSidecarWithRetry({
    ownerNonce: 'gen-3',
    attempts: 4,
    delayMs: 1,
...
test('readOwnedSidecarWithRetry tolerates a throwing reader (treats as absent)', async () => {
  let reads = 0
  const result = await readOwnedSidecarWithRetry({
    ownerNonce: 'gen-4',
    attempts: 2,
