...
var require_session_windows = __commonJS({
  "electron/session-windows.cjs"(exports2, module2) {
    ...
    function buildSessionWindowUrl2(sessionId, { devServer, rendererIndexPath, watch, newSession, profile } = {}) {
      const queryParams = new URLSearchParams({ win: "secondary" });
      const profileKey = typeof profile === "string" ? profile.trim() : "";
      if (newSession) {
        queryParams.set("new", "1");
      }
      if (watch) {
        queryParams.set("watch", "1");
      }
      if (!newSession && profileKey) {
        queryParams.set("profile", profileKey);
      }
...
var require_backend_pool = __commonJS({
  "electron/backend-pool.cjs"(exports2, module2) {
    "use strict";
    var fs2 = require("node:fs");
    var DEFAULT_BACKEND_POOL_SETTINGS = Object.freeze({
      maxProfileBackends: 3,
      maxSessionBackendsPerProfile: 10,
      spawnConcurrency: 10,
      idleMs: 10 * 6e4,
      keepaliveFreshMs: 9e4,
    ...
    var BACKEND_ISOLATION_MODES = ["profile", "hybrid", "session"];
    ...
    function resolveBackendPoolSettings(config = {}) {
      ...
      return {
        ...
        idleMs: numberSetting(raw.idle_ms, DEFAULT_BACKEND_POOL_SETTINGS.idleMs, { min: 6e4 }),
        keepaliveFreshMs: numberSetting(raw.keepalive_fresh_ms, DEFAULT_BACKEND_POOL_SETTINGS.keepaliveFreshMs, { min: 1e3 }),
        healthTimeoutMs: numberSetting(raw.health_timeout_ms, DEFAULT_BACKEND_POOL_SETTINGS.healthTimeoutMs, { min: 250 }),
        startupTimeoutMs: numberSetting(raw.startup_timeout_ms, DEFAULT_BACKEND_POOL_SETTINGS.startupTimeoutMs, { min: 45e3 }),
    ...
    function readBackendPoolSettingsFromConfig2(configPath) {
      let text;
      try {
        text = fs2.readFileSync(configPath, "utf8");
      } catch {
    ...
    function resolveBackendTarget2(profile, opts, backendIsolation) {
    ...
    function backendScopeForTarget2(target, primaryProfileKey2) {
      const primary = normalizeProfileKey(primaryProfileKey2);
    ...
    function normalizeManagementScope2(scope, primaryProfileKey2) {
    ...
    var DesktopBackendPool2 = class {
      constructor(options = {}) {
        ...
        this.forgetEntry = options.forgetEntry || (() => void 0);
        this.tombstoneEntry = options.tombstoneEntry || (() => void 0);
      ...
      _normalizeTarget(target) {
        ...
        if (target && typeof target === "object" && target.isolation === "session") {
          const profileKey2 = normalizeProfileKey(target.profile, primary);
          const sid = target.sessionId != null ? String(target.sessionId).trim() : "";
          if (!sid) throw new Error("session isolation requires a sessionId");
          return { scope: "session", profile: profileKey2, sessionId: sid, key: `session:${profileKey2}:${sid}` };
        }
      ...
      async ensureBackend(target) {
        ...
        if (existing) {
          ...
          if (!confirmed) {
            throw new Error(
              `Profile backend "${key}" is unresponsive and could not be confirmed stopped; not spawning a same-id replacement. Retry shortly \u2014 the reaper will clear the wedged backend.`
            );
          }
        }
        ...
        entry.connectionPromise = this.spawnBackend(key, entry).then((connection) => {
      ...
      evictLru(keep) {
        ...
        const now = this.now();
        const evictable = [...this.entries.entries()].filter(([, entry]) => now - (entry.lastActiveAt || 0) > this.settings.keepaliveFreshMs).sort((a, b) => (a[1].lastActiveAt || 0) - (b[1].lastActiveAt || 0));
      ...
      confirmTeardownInBackground(profile) {
        Promise.resolve(this.teardownBackendAndWait(profile)).catch(() => void 0);
      }
      startIdleReaper() {
      ...
      reapIdle() {
        ...
        for (const [profile, entry] of [...this.entries.entries()]) {
          if (now - (entry.lastActiveAt || 0) > this.settings.idleMs) {
            this.log(`Reaping idle profile backend "${profile}" (idle > ${Math.round(this.settings.idleMs / 1e3)}s)`);
            this.confirmTeardownInBackground(profile);
          }
        }
      ...
      async teardownBackendAndWait(target) {
        ...
        const entry = this.entries.get(key);
        if (!entry) return { confirmed: true };
        this.entries.delete(key);
        entry.lifecycle = "stopping";
        this.stopEntry(key, entry);
    ...
    module2.exports = {
      ...
      DesktopBackendPool: DesktopBackendPool2,
      backendScopeForTarget: backendScopeForTarget2,
      backendScopeKey,
      normalizeManagementScope: normalizeManagementScope2,
      normalizeProfileKey,
      readBackendPoolSettingsFromConfig: readBackendPoolSettingsFromConfig2,
      resolveBackendPoolSettings,
      resolveBackendTarget: resolveBackendTarget2
    };
  }
});
var require_backend_registry = __commonJS({
  "electron/backend-registry.cjs"(exports2, module2) {
    "use strict";
    var fs2 = require("node:fs");
    var path2 = require("node:path");
    var REGISTRY_SCHEMA_VERSION = 1;
    var BACKEND_OWNER_ARG = "--hermes-desktop-owner";
    var SECRET_KEY_RE = /(token|secret|api[_-]?key|authorization|bearer|password|credential)/i;
    var PROCESS_START_TIME_TOLERANCE_SECONDS = 2;
    ...
    function normalizeHermesRoot(hermesHome, { pathModule = path2 } = {}) {
      if (!hermesHome) return hermesHome;
      const resolved = pathModule.resolve(String(hermesHome));
      const parent = pathModule.dirname(resolved);
      if (pathModule.basename(parent).toLowerCase() === "profiles") {
    ...
    function desktopBackendRegistryPath2(hermesHome, { pathModule = path2 } = {}) {
      return pathModule.join(normalizeHermesRoot(hermesHome, { pathModule }), "runtime", "desktop-backends.json");
    }
    function backendDescriptorId2(descriptor) {
    ...
    function backendOwnerArg2(ownerNonce) {
    ...
    function registryRecordForDescriptor2(descriptor, options = {}) {
      const scope = descriptor?.scope;
      const profile = String(descriptor?.profile || "").trim();
      const sessionId = descriptor?.sessionId ? String(descriptor.sessionId) : null;
      const id = descriptor.id || backendDescriptorId2({ scope, profile, sessionId });
      ...
      return {
        ...
        startedAt: options.startedAt || descriptor?.startedAt || (/* @__PURE__ */ new Date()).toISOString(),
    ...
    function readBackendRegistry2(file) {
      try {
    ...
    function writeBackendRegistry2(file, records) {
...
`, "utf8");
    ...
    function argvHasOwnerNonce(argv, ownerNonce) {
      ...
      const expectedEquals = backendOwnerArg2(ownerNonce);
    ...
    var _UNHEALTHY_LIFECYCLES = ["unresponsive", "failed", "hung"];
    function reconcileBackendRegistry(records, processTable, options = {}) {
      const claimedIds = options.claimedIds instanceof Set ? options.claimedIds : new Set(Array.isArray(options.claimedIds) ? options.claimedIds : []);
    ...
    function buildProcessTableFromRows(rows, options = {}) {
      ...
      for (const row of list) {
        ...
        const cmdlineReadable = row.cmdlineReadable !== false && cmdline.length > 0 ? true : Boolean(row.cmdlineReadable);
        const installRoot = expectedInstallRoot && cmdline.includes(expectedInstallRoot) ? expectedInstallRoot : null;
        table[String(pid)] = {
          pid,
    ...
    module2.exports = {
      ...
      backendDescriptorId: backendDescriptorId2,
      backendOwnerArg: backendOwnerArg2,
      buildProcessTableFromRows,
      desktopBackendRegistryPath: desktopBackendRegistryPath2,
      hasSecretLikeRegistryField,
      normalizeProcessStartTime,
      readBackendRegistry: readBackendRegistry2,
      reconcileBackendRegistry,
      registryRecordForDescriptor: registryRecordForDescriptor2,
      verifyBackendOwnership,
      writeBackendRegistry: writeBackendRegistry2
    };
  }
});
var require_backend_reconcile = __commonJS({
  "electron/backend-reconcile.cjs"(exports2, module2) {
    "use strict";
    var { buildProcessTableFromRows, reconcileBackendRegistry } = require_backend_registry();
    function createBackendReconciler2(deps) {
    ...
    module2.exports = { createBackendReconciler: createBackendReconciler2 };
  }
});
var require_backend_kill = __commonJS({
  "electron/backend-kill.cjs"(exports2, module2) {
    "use strict";
    function confirmProcessKilled2(pid, { kill, isAlive }) {
    ...
    module2.exports = { confirmProcessKilled: confirmProcessKilled2 };
  }
});
var require_backend_registry_store = __commonJS({
  "electron/backend-registry-store.cjs"(exports2, module2) {
    "use strict";
    function createRegistryStore2({ readRecords, writeRecords }) {
      ...
      function upsert(record) {
        return runExclusive(() => {
          ...
          if (existing && existing.lifecycle === "failed" && existing.ownerNonce && record.ownerNonce && existing.ownerNonce !== record.ownerNonce) {
      ...
      function remove(id, { ownerNonce = null } = {}) {
        return runExclusive(() => {
          ...
          if (!nonceMatches(match, ownerNonce)) return { removed: false };
    ...
    module2.exports = { createRegistryStore: createRegistryStore2 };
  }
});
var require_backend_spawn_guard = __commonJS({
  "electron/backend-spawn-guard.cjs"(exports2, module2) {
    "use strict";
    async function reserveThenSpawn2({ reserve, spawnFn, onConflict, onReservationFailed }) {
    ...
    module2.exports = { reserveThenSpawn: reserveThenSpawn2 };
  }
});
var require_primary_teardown_latch = __commonJS({
  "electron/primary-teardown-latch.cjs"(exports2, module2) {
    "use strict";
    ...
    function createPrimaryTeardownLatch2(options = {}) {
    ...
    module2.exports = { createPrimaryTeardownLatch: createPrimaryTeardownLatch2, childHasExited };
  }
});
...
var {
  ...
  resolveBackendTarget: resolveBackendTargetPure
} = require_backend_pool();
var {
  backendDescriptorId,
  backendOwnerArg,
...
} = require_backend_registry();
var { createBackendReconciler } = require_backend_reconcile();
var { confirmProcessKilled } = require_backend_kill();
var { createRegistryStore } = require_backend_registry_store();
var { reserveThenSpawn } = require_backend_spawn_guard();
var { createPrimaryTeardownLatch } = require_primary_teardown_latch();
...
var BACKEND_POOL_SETTINGS = readBackendPoolSettingsFromConfig(path.join(HERMES_HOME, "config.yaml"));
var DESKTOP_INSTANCE_ID = crypto.randomUUID();
var DESKTOP_BACKEND_REGISTRY_PATH = desktopBackendRegistryPath(HERMES_HOME);
var DESKTOP_BACKEND_SIDECAR_DIR = path.join(HERMES_HOME, "runtime", "desktop-backends");
var profileBackendPool = new DesktopBackendPool({
  settings: BACKEND_POOL_SETTINGS,
  primaryProfileKey,
  startPrimary: () => startHermes(),
  spawnBackend: (profile, entry) => spawnPoolBackend(profile, entry),
  healthProbe: async (connection) => {
    const base = String(connection?.baseUrl || "").replace(/\/+$/, "");
    if (!base) return false;
    if (connection.authMode === "oauth") {
      await fetchPublicJson(`${base}/api/status`, { timeoutMs: BACKEND_POOL_SETTINGS.healthTimeoutMs });
    } else {
      await fetchJson(`${base}/api/status`, connection.token, { timeoutMs: BACKEND_POOL_SETTINGS.healthTimeoutMs });
    }
    return true;
  },
  waitForBackendExit,
  stopEntry: stopPoolEntry,
  forgetEntry: forgetPoolEntry,
  tombstoneEntry: (_profile, entry) => tombstonePoolBackendRecord(entry),
  log: rememberLog
});
...
var primaryBackendPhase = "missing";
var primaryTeardownLatch = createPrimaryTeardownLatch();
...
function safeBackendArtifactName(value) {
  return String(value || "backend").replace(/[^a-zA-Z0-9_.-]+/g, "_");
}
function makeDesktopBackendSidecarPath(backendId) {
  fs.mkdirSync(DESKTOP_BACKEND_SIDECAR_DIR, { recursive: true });
  return path.join(DESKTOP_BACKEND_SIDECAR_DIR, `${safeBackendArtifactName(backendId)}.sidecar.json`);
}
...
var desktopBackendRegistryStore = createRegistryStore({
  readRecords: () => readBackendRegistry(DESKTOP_BACKEND_REGISTRY_PATH).records,
  writeRecords: (records) => writeBackendRegistry(DESKTOP_BACKEND_REGISTRY_PATH, records)
});
function upsertDesktopBackendRegistryRecord(record) {
  return desktopBackendRegistryStore.upsert(registryRecordForDescriptor(record)).catch((error) => rememberLog(`Failed to update desktop backend registry: ${error.message}`));
}
async function reserveDesktopBackendRegistryRecord(record) {
  try {
    return await desktopBackendRegistryStore.upsert(registryRecordForDescriptor(record));
  } catch (error) {
    rememberLog(`Failed to reserve desktop backend registry record ${record.id}: ${error.message}`);
    return { upserted: false, error: true, id: record.id };
  }
}
async function removeDesktopBackendRegistryRecord(id, sidecarPath, opts = {}) {
  let removedMatchingRow = false;
  try {
    const result = await desktopBackendRegistryStore.remove(id, { ownerNonce: opts.ownerNonce || null });
    removedMatchingRow = Boolean(result && result.removed);
  } catch (error) {
    rememberLog(`Failed to prune desktop backend registry: ${error.message}`);
  }
  if (removedMatchingRow && sidecarPath) {
    try {
      fs.unlinkSync(sidecarPath);
    } catch (error) {
      if (error?.code !== "ENOENT") rememberLog(`Failed to remove backend sidecar ${sidecarPath}: ${error.message}`);
    }
  }
}
function tombstonePoolBackendRecord(entry) {
  if (!entry?.registryId) return Promise.resolve();
  return desktopBackendRegistryStore.tombstone(entry.registryId, {
    ownerNonce: entry.ownerNonce || null,
    patch: { lifecycle: "failed", tombstonedAt: (/* @__PURE__ */ new Date()).toISOString() }
  }).catch((error) => rememberLog(`Failed to tombstone desktop backend registry record ${entry.registryId}: ${error.message}`));
}
function forgetPoolEntry(_profile, _entry) {
}
function stopPoolEntry(_profile, entry) {
  if (entry?.process && !entry.process.killed) {
    try {
      entry.process.kill("SIGTERM");
    } catch {
...
function enumerateProcessRows() {
  if (IS_WINDOWS) {
    const script = [
      "Get-CimInstance Win32_Process | ForEach-Object {",
      "  $ct = $null;",
      "  if ($_.CreationDate) { $ct = [int64](($_.CreationDate.ToUniversalTime() - [datetime]'1970-01-01T00:00:00Z').TotalSeconds) };",
      "  [pscustomobject]@{ pid = $_.ProcessId; ppid = $_.ParentProcessId; createTime = $ct;",
      "    cmdline = $_.CommandLine; cmdlineReadable = ($_.CommandLine -ne $null) }",
      "} | ConvertTo-Json -Compress -Depth 3"
    ].join(" ");
    const out2 = execFileSync(
      "powershell",
      ["-NoProfile", "-NonInteractive", "-Command", script],
      hiddenWindowsChildOptions({ encoding: "utf8", maxBuffer: 32 * 1024 * 1024, timeout: 2e4 })
    );
    const parsed = JSON.parse(out2 || "[]");
    return Array.isArray(parsed) ? parsed : [parsed];
  }
  const out = execFileSync("ps", ["-Ao", "pid=,ppid=,command="], { encoding: "utf8", maxBuffer: 32 * 1024 * 1024, timeout: 2e4 });
  return out.split(/\r?\n/).map((line) => {
    const match = line.match(/^\s*(\d+)\s+(\d+)\s+(.*)$/);
    if (!match) return null;
    return { pid: Number(match[1]), ppid: Number(match[2]), createTime: null, cmdline: match[3], cmdlineReadable: true };
  }).filter(Boolean);
}
...
function collectClaimedBackendIds() {
  const ids = /* @__PURE__ */ new Set();
  for (const entry of profileBackendPool.values()) {
    if (entry && entry.registryId) ids.add(entry.registryId);
  }
  return ids;
}
function loadSidecarsForRecords(records) {
  const sidecars = {};
  for (const record of records) {
    if (record && record.sidecarPath) {
      const data = readJsonFileIfExists(record.sidecarPath);
      if (data) sidecars[record.sidecarPath] = data;
    }
  }
  return sidecars;
}
function removeReconciledSidecar(sidecarPath) {
  if (!sidecarPath) return;
  try {
    fs.unlinkSync(sidecarPath);
  } catch (error) {
    if (error?.code !== "ENOENT") rememberLog(`[reconcile] failed to remove sidecar ${sidecarPath}: ${error.message}`);
  }
}
var backendReconciler = createBackendReconciler({
  readRegistry: () => readBackendRegistry(DESKTOP_BACKEND_REGISTRY_PATH),
  // Generation-safe apply through the serialized store: it re-reads under the
  // lock and matches by (id, ownerNonce), so a fresh same-id spawn that landed
  // while the reaper was deciding is never clobbered.
  commitReconcile: (decisions) => desktopBackendRegistryStore.commitReconcile(decisions),
  enumerateProcessRows,
  killBackendTree,
  collectClaimedIds: collectClaimedBackendIds,
  loadSidecars: loadSidecarsForRecords,
  removeSidecar: removeReconciledSidecar,
  expectedInstallRoot: ACTIVE_HERMES_ROOT,
  currentInstanceId: DESKTOP_INSTANCE_ID,
  log: rememberLog
});
function reconcileDesktopBackends(reason) {
  return backendReconciler.reconcile(reason);
}
function primaryBackendState() {
  if (bootstrapFailure || backendStartFailure) {
    return "failed";
  }
  if (primaryTeardownLatch.isActive()) {
    return "failed";
  }
  return primaryBackendPhase;
}
function managedBackendStatus(scope) {
  const norm = normalizeManagementScope(scope, primaryProfileKey());
  if (norm.scope === "primary") {
    return {
      ok: true,
      scope: "primary",
      profile: norm.profile,
      state: primaryBackendState()
    };
  }
  const described = profileBackendPool.describeBackend(norm.target);
  return {
    ok: true,
    scope: norm.scope,
    profile: norm.profile,
    ...norm.sessionId ? { sessionId: norm.sessionId } : {},
    state: described.state
  };
}
async function restartManagedBackend(scope) {
  const norm = normalizeManagementScope(scope, primaryProfileKey());
  if (norm.scope === "primary") {
    const confirmed2 = await teardownPrimaryBackendAndWait();
    if (!confirmed2) {
      return {
        ok: false,
        scope: "primary",
        profile: norm.profile,
        state: "failed",
        error: "teardown-unconfirmed"
      };
    }
    await startHermes();
    return managedBackendStatus(scope);
  }
  const { confirmed } = await profileBackendPool.teardownBackendAndWait(norm.target);
  if (!confirmed) {
    return {
      ok: false,
      scope: norm.scope,
  ...
  if (norm.scope === "session") {
    await ensureBackend(norm.profile, { isolation: "session", sessionId: norm.sessionId });
  } else {
    await ensureBackend(norm.profile);
  }
  return managedBackendStatus(scope);
}
async function stopManagedBackend(scope) {
  const norm = normalizeManagementScope(scope, primaryProfileKey());
  if (norm.scope === "primary") {
    const confirmed2 = await teardownPrimaryBackendAndWait();
    if (!confirmed2) {
      return { ok: false, confirmed: false, error: "teardown-unconfirmed" };
    }
...
async function reconcileManagedBackends() {
  const result = await reconcileDesktopBackends("ipc");
  if (!result) {
    return { ok: false, error: "reconcile-skipped" };
  }
  return { ok: true, reaped: result.reaped, pruned: result.pruned, tombstoned: result.tombstoned, kept: result.kept };
}
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  pids.push(...profileBackendPool.childPids());
  ...
  signalAllPoolBackendsForShutdown();
...
async function applyUpdatesPosixInApp() {
  ...
  desktopChildPids.push(...profileBackendPool.childPids());
...
async function freshGatewayWsUrl(profile, opts) {
  const connection = await ensureBackend(profile, opts);
  if (connection.authMode === "oauth") {
...
async function teardownPrimaryBackendAndWait() {
  let dying = hermesProcess && !hermesProcess.killed ? hermesProcess : null;
  if (!dying && primaryTeardownLatch.isActive()) {
    dying = primaryTeardownLatch.child();
  }
  resetHermesConnection();
  const confirmed = await waitForBackendExit(dying);
  if (confirmed) {
    primaryTeardownLatch.clear();
    primaryBackendPhase = "missing";
  } else {
    primaryTeardownLatch.arm({ child: dying, pid: dying ? dying.pid : null, since: Date.now(), reason: "teardown-unconfirmed" });
    primaryBackendPhase = "failed";
    watchLatchedPrimaryChildExit(dying);
  }
  return confirmed;
}
function watchLatchedPrimaryChildExit(child) {
  if (!child || typeof child.once !== "function") {
    return;
  }
  child.once("exit", () => {
    if (primaryTeardownLatch.child() === child) {
      primaryTeardownLatch.clear();
      if (primaryBackendPhase === "failed") {
        primaryBackendPhase = "missing";
      }
    }
  });
...
function resolveBackendTarget(profile, opts) {
  return resolveBackendTargetPure(profile, opts, BACKEND_POOL_SETTINGS.backendIsolation);
}
async function ensureBackend(profile, opts) {
  const target = resolveBackendTarget(profile, opts);
  const connection = await profileBackendPool.ensureBackend(target);
  return { ...connection, backendScope: backendScopeForTarget(target, primaryProfileKey()) };
}
function touchPoolBackend(profile, opts) {
  profileBackendPool.touchBackend(resolveBackendTarget(profile, opts));
}
async function spawnPoolBackend(scopeKey, entry) {
  const scope = entry.scope || "profile";
  const profile = entry.profile || scopeKey;
  ...
  const registryId = backendDescriptorId({ scope, profile, sessionId });
  const ownerNonce = crypto.randomUUID();
  const sidecarPath = makeDesktopBackendSidecarPath(registryId);
  const startedAt = (/* @__PURE__ */ new Date()).toISOString();
  entry.registryId = registryId;
  entry.ownerNonce = ownerNonce;
  entry.sidecarPath = sidecarPath;
  entry.desktopInstanceId = DESKTOP_INSTANCE_ID;
  entry.startedAt = startedAt;
  const dashboardArgs = ["--profile", profile, backendOwnerArg(ownerNonce), "dashboard", "--no-open", "--host", "127.0.0.1", "--port", "0"];
  const backend = await reserveThenSpawn({
    reserve: () => reserveDesktopBackendRegistryRecord({
      scope,
      profile,
      sessionId,
      id: registryId,
      ...
      installRoot: ACTIVE_HERMES_ROOT,
      lifecycle: "starting",
    ...
    onConflict: () => {
      throw new Error(
        `Cannot start backend for profile "${profile}": a previous unresponsive generation is still being reaped. Retry shortly.`
      );
    },
    onReservationFailed: () => {
      throw new Error(
        `Cannot start backend for profile "${profile}": failed to reserve its registry record. Retry shortly.`
      );
    },
    spawnFn: () => ensureRuntime(resolveHermesBackend(dashboardArgs))
  });
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        HERMES_DESKTOP: "1",
        HERMES_DESKTOP_BACKEND_SIDECAR: sidecarPath,
        HERMES_DESKTOP_BACKEND_OWNER: ownerNonce,
        HERMES_DESKTOP_BACKEND_SCOPE: scope,
        HERMES_DESKTOP_BACKEND_PROFILE: profile,
        HERMES_DESKTOP_BACKEND_ID: registryId,
        // Session backends carry their stored session id (internal metadata, not
        // a secret) so logs/sidecar can attribute the process to its session.
        ...isSession ? { HERMES_DESKTOP_BACKEND_SESSION_ID: sessionId } : {},
        // Session backends run MCP in PROXY mode so they never spawn their own
        // stdio MCP servers (discover_mcp_tools short-circuits to the broker
        // catalog path). Broker connection details (URL + ephemeral token) are
        // wired in a later pass; until then proxy mode registers no tools but
        // guarantees no duplicate MCP stacks. No tokens are placed in argv, the
        // registry, or the sidecar.
        ...isSession ? { HERMES_MCP_MODE: "proxy" } : {},
        HERMES_DESKTOP_INSTANCE_ID: DESKTOP_INSTANCE_ID,
        HERMES_DESKTOP_INSTALL_ROOT: ACTIVE_HERMES_ROOT,
  ...
  entry.token = token;
  entry.port = null;
  upsertDesktopBackendRegistryRecord({
    scope,
    profile,
    sessionId,
    id: registryId,
  ...
  child.once("error", (error) => {
    ...
    profileBackendPool.forgetBackend(scopeKey);
    removeDesktopBackendRegistryRecord(registryId, sidecarPath, { ownerNonce });
    rejectStart?.(error);
  });
  ...
  ready = true;
  const sidecar = readJsonFileIfExists(sidecarPath);
  const sidecarVerified = sidecar && sidecar.kind === "hermes-desktop-backend" && sidecar.ownerNonce === ownerNonce;
  if (sidecar && !sidecarVerified) {
    rememberLog(`Ignoring backend sidecar for profile "${profile}": owner nonce did not match registry record.`);
  }
  upsertDesktopBackendRegistryRecord({
    scope,
    profile,
    sessionId,
    id: registryId,
    pid: Number.isInteger(sidecarVerified ? sidecar.pid : child.pid) ? sidecarVerified ? sidecar.pid : child.pid : null,
    processStartTime: sidecarVerified ? sidecar.processStartTime : null,
    installRoot: sidecarVerified ? sidecar.installRoot : ACTIVE_HERMES_ROOT,
    lifecycle: "ready",
    desktopInstanceId: DESKTOP_INSTANCE_ID,
    ownerNonce,
    sidecarPath,
...
async function stopPoolBackend(profile) {
  return profileBackendPool.teardownBackendAndWait(profile);
}
async function teardownPoolBackendAndWait(profile) {
  await profileBackendPool.teardownBackendAndWait(profile);
}
function signalAllPoolBackendsForShutdown() {
  profileBackendPool.stopAllBackends();
}
...
async function startHermes() {
  ...
  if (primaryTeardownLatch.isActive()) {
    throw new Error("primary backend teardown unconfirmed \u2014 refusing to start a replacement while the previous backend may still be running");
  }
  if (connectionPromise) return connectionPromise;
  primaryBackendPhase = "starting";
...
ipcMain.handle("hermes:connection", async (_event, profile, opts) => ensureBackend(profile, opts));
...
ipcMain.handle("hermes:backend:touch", async (_event, profile, opts) => {
  touchPoolBackend(profile, opts);
  return { ok: true };
});
ipcMain.handle("hermes:backend:status", async (_event, scope) => {
  try {
    return managedBackendStatus(scope);
  } catch (error) {
...
ipcMain.handle("hermes:backend:restart", async (_event, scope) => {
  try {
    return await restartManagedBackend(scope);
  } catch (error) {
    return { ok: false, error: error instanceof Error ? error.message : String(error) };
  }
});
ipcMain.handle("hermes:backend:stop", async (_event, scope) => {
  try {
    return await stopManagedBackend(scope);
  } catch (error) {
    return { ok: false, error: error instanceof Error ? error.message : String(error) };
  }
});
ipcMain.handle("hermes:backend:reconcile", async () => {
  try {
    return await reconcileManagedBackends();
  } catch (error) {
    return { ok: false, error: error instanceof Error ? error.message : String(error) };
  }
});
ipcMain.handle("hermes:gateway:ws-url", async (_event, profile, opts) => freshGatewayWsUrl(profile, opts));
...
ipcMain.handle("hermes:bootstrap:reset", async () => {
  ...
  const confirmed = await teardownPrimaryBackendAndWait();
  if (!confirmed) {
    rememberLog("[bootstrap] reset aborted: primary teardown unconfirmed");
    return { ok: false, error: "teardown-unconfirmed" };
  }
...
ipcMain.handle("hermes:bootstrap:repair", async () => {
  ...
  const confirmed = await teardownPrimaryBackendAndWait();
  if (!confirmed) {
    rememberLog("[bootstrap] repair aborted: primary teardown unconfirmed");
    return { ok: false, error: "teardown-unconfirmed" };
  }
  try {
...
ipcMain.handle("hermes:profile:set", async (_event, name) => {
  ...
  await reconcileDesktopBackends("profile-switch");
  await teardownPrimaryBackendAndWait();
  mainWindow?.reload();
  return { profile: next };
});
...
app.whenReady().then(async () => {
  ...
  try {
    await reconcileDesktopBackends("startup");
  } catch (error) {
    rememberLog(`[reconcile:startup] unexpected failure: ${error.message}`);
  }
  createWindow();
