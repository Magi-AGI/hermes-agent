...
def _is_windows_user_home_root(path: str) -> bool:
    """Return True for broad Windows home roots like C:/Users/TestUser or /c/Users/TestUser."""
...
def _windows_cloud_search_policy(path: str) -> _SearchCloudPolicy:
    ...
    shell I/O, and add excludes for broad home-root searches.  Explicit,
    non-cloud project/repo paths are left untouched.
    """
    parts = _windows_path_parts(path)
...
class ShellFileOperations(FileOperations):
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content",
        ...
        # Windows cloud-placeholder safety. Policy runs just after _expand_path
        # (a tilde path may already have cost one `echo $HOME`), but before the
        # path-existence probe and any search traversal: explicit cloud roots
        # are refused up front, and broad home-root searches get cloud-dir
        # excludes + a warning, so we never hydrate OneDrive/Dropbox offline
        # placeholders just by walking or stat-ing them.
        if path in {"", ".", "./"}:
    ...
    def _search_files(
        ...
        # Cloud safety: PRUNE cloud-synced directories so find never descends
        # into (or stats) OneDrive/Dropbox/iCloud/Creative Cloud placeholder
        # trees.  A -prune (rather than -not -path output filtering) is what
        # actually prevents Files-On-Demand hydration during a broad home-root
        # search — output filtering still traverses/stats the tree first.
        # The pruned branch carries no action, so pruned dirs are not printed;
        # the match branch therefore carries an explicit action (-printf /
        # -print) to suppress find's default -print of pruned entries.
        cloud_prune_prefix = ""
        if exclude_windows_cloud_dirs:
            cloud_name_tests = " -o ".join(
                f"-name '{glob}'" for glob in _WINDOWS_CLOUD_DIR_GLOBS
            )
            cloud_prune_prefix = f"\\( -type d \\( {cloud_name_tests} \\) \\) -prune -o "
        ...
        if cloud_prune_prefix:
            cmd = (
                f"find {self._escape_shell_arg(path)} {cloud_prune_prefix}"
